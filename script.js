...
function renderCart(){cartItemsUl.innerHTML='';cart.forEach((item,i)=>{let li=document.createElement('li');li.textContent=item;let rm=document.createElement('span');rm.textContent='✖';rm.style.cursor='pointer';rm.addEventListener('click',()=>{cart.splice(i,1);renderCart();});li.appendChild(rm);cartItemsUl.appendChild(li);});}
...
if(prevBtn&&nextBtn){prevBtn.addEventListener('click',()=>moveCarousel(-carousel.offsetWidth*0.33));nextBtn.addEventListener('click',()=>moveCarousel(carousel.offsetWidth*0.33));}
